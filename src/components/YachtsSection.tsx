...
function fromDB(listings: DBListing[]): Card[] {
  return listings.map((l) => ({
    ...
    tagline: l.tagline ?? '',
    detail: l.details?.model ?? l.location ?? '',
    image: l.images?.[0] ?? 'https://images.unsplash.com/photo-1567899378494-47b22a2ae96a?w=900&q=90&auto=format&fit=crop',
  }))
}
...
function YachtCard({ card, index }: { card: Card; index: number }) {
  ...
  return (
    ...
    >
      <Link href={card.href} className="block">
        <div className="relative overflow-hidden" style={{ height: "clamp(280px, 60vw, 460px)" }}>
          <Image
            src={card.image}
            alt={card.name}
            fill
          ...
          <div className="absolute bottom-0 left-0 right-0 px-6 pb-7">
            <p
              ...
              style={{ fontFamily: "var(--font-cormorant)", fontSize: "clamp(1.4rem, 4.5vw, 2rem)", letterSpacing: "0.04em" }}
            >
              {card.name}
            </p>
